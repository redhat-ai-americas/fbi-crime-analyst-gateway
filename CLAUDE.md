...
| `AGENT_NAME` | No | `fbi-crime-analyst-gateway` | Name in agent card |
...
helm upgrade --install my-gateway chart/ \
  ...
  --set image.repository=image-registry.openshift-image-registry.svc:5000/my-namespace/fbi-crime-analyst-gateway
```

## Sentinel Values

This is a template repository. The string `fbi-crime-analyst-gateway` appears throughout and is replaced with the actual project name during scaffolding by `fips-agents create gateway <name>`.
